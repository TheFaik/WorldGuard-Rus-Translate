    ...
    public void onProjectile(DamageEntityEvent event) {
        ...
        if (blockedEffect != null) {
            ...
            if (player != null) {
                ...
                player.sendMessage(ChatColor.RED + "Извините, стрелы с "
                ...
                if (player != null) {
                    if (getPlugin().hasPermission(player, "worldguard.override.potions")) {
                        if (wcfg.blockPotionsAlways && (item.getType() == Material.SPLASH_POTION
                                || item.getType() == Material.LINGERING_POTION)) {
                            player.sendMessage(ChatColor.RED + "Извините, зелья с " +
                                    blockedEffect.getName() + " нельзя бросать, " +
                                    "даже если у вас есть разрешение на обход этого, " +
                                    "из-за ограничений (и из-за того, что включена сверхнадежная блокировка зелий).");
                            event.setCancelled(true);
                        }
                    } else {
                        player.sendMessage(ChatColor.RED + "Извините, зелья с "
