...
public class CancelReport implements Report {
    ...
    public String toString() {
        if (!cancels.isEmpty()) {
            ...
            return "Ни один плагин не отменил событие. Другие причины отмены: " +
                    "(1) Bukkit может использовать другое событие для этого действия " +
                    " (пример: для вёдер есть свои события); или " +
                    "(2) Защита спавна Minecraft не была отключена.";
        }
    }

    private static String getCancelText(boolean flag) {
        return flag ? "ЗАБЛОКИРОВАН" : "РАЗРЕШЁН";
    }
