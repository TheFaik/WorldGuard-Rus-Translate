...
public abstract class AbstractSessionManager implements SessionManager {
    ...
    public boolean registerHandler(Handler.Factory<? extends Handler> factory, @Nullable Handler.Factory<? extends Handler> after) {
        ...
        WorldGuard.logger.log(Level.INFO, "Регистрация обработчика сессии "
    ...
    public boolean unregisterHandler(Handler.Factory<? extends Handler> factory) {
        if (defaultHandlers.contains(factory)) {
            WorldGuard.logger.log(Level.WARNING, "Кто-то отменяет регистрацию стандартного обработчика WorldGuard: "
                    + factory.getClass().getEnclosingClass().getName() + ". Это может привести к прекращению работы некоторых частей WorldGuard");
        } else {
            WorldGuard.logger.log(Level.INFO, "Отмена регистрации обработчика сессии "
                    + factory.getClass().getEnclosingClass().getName());
        }
