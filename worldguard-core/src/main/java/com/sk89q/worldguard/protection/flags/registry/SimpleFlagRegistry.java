...
public class SimpleFlagRegistry implements FlagRegistry {
    ...
    public void register(Flag<?> flag) throws FlagConflictException {
        synchronized (lock) {
            if (initialized) {
                throw new IllegalStateException("Регистрация новых флагов в данный момент невозможна");
            }
    ...
    private Flag<?> forceRegister(Flag<?> flag) throws FlagConflictException {
        checkNotNull(flag, "флаг");
        checkNotNull(flag.getName(), "flag.getName()");

        synchronized (lock) {
            String name = flag.getName().toLowerCase();
            if (flags.containsKey(name)) {
                throw new FlagConflictException("Флаг с таким названием уже существует " + name);
            }
            ...
            if (flag != null) {
                try {
                    ...
                    if (unmarshalled != null) {
                        ...
                        log.warning("Не удалось разобрать флаг '" + flag.getName() + "' с использованием значения '" + entry.getValue() + "'");
                    }
                } catch (Throwable e) {
                    log.log(Level.WARNING, "Не удалось десериализовать значение флага для " + flag, e);
                }
            }
        }
