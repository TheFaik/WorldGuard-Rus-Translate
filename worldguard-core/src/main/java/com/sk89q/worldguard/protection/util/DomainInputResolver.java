    ...
    public DefaultDomain call() throws UnresolvedNamesException {
        ...
        if (!namesToQuery.isEmpty()) {
            try {
                ...
                throw new UnresolvedNamesException("Служба поиска UUID не сработала, поэтому введенные имена не удалось превратить в UUIDs");
        ...
        if (!namesToQuery.isEmpty()) {
            throw new UnresolvedNamesException("Не удалось разрешить имена " + Joiner.on(", ").join(namesToQuery));
        }
