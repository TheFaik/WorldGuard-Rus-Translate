...
public class UUIDMigration extends AbstractMigration {
    ...
    protected void migrate(RegionDatabase store) throws MigrationException {
        log.log(Level.INFO, "Миграция регионов в '" + store.getName() + "' для преобразования имен -> UUID...");
        ...
        try {
            ...
            throw new MigrationException("Не удалось сохранить данные регионов после миграции мира '" + store.getName() + "'", e);
        }
    }
    ...
    private boolean migrate(Collection<ProtectedRegion> regions) throws MigrationException {
        ...
        if (!names.isEmpty()) {
            ...
            try {
                ...
                log.log(Level.INFO, "Разрешение " + names.size() + " имен(и) в UUID... это может занять некоторое время.");
                ...
                throw new MigrationException("Сервис имен -> UUID не сработал", e);
            ...
            log.log(Level.INFO, "UUID разрешены... теперь перенос всех регионов на UUID, где это возможно...");
            convert(regions);
        ...
        if (!unresolvedNames.isEmpty()) {
            if (keepUnresolvedNames) {
                log.log(Level.WARNING,
                        "Некоторые имена участников и владельцев, похоже, не существуют или не владеют Minecraft, поэтому они " +
                                "не могли быть преобразованы в UUID. Они были оставлены в виде имен, но преобразование может " +
                                "повторно запущено с 'keep-names-that-lack-uuids', установленным в false в конфигурации, " +
                                "чтобы удалить эти имена. Оставление имен означает, что кто-то может зарегистрироваться с одним из " +
                                "этих имен в будущем и стать этим игроком.");
            } else {
                log.log(Level.WARNING,
                        "Некоторые имена участников и владельцев, похоже, не существуют или не владеют Minecraft, поэтому они " +
                                "не могли быть преобразованы в UUIDs. Эти имена были удалены.");
            }
        }
    }
    ...
    private class ResolvedNamesTimerTask extends TimerTask {
        @Override
        public void run() {
            log.info("UUID были найдены для " + resolvedNames.size() + " имен(и)...");
        }
    }
