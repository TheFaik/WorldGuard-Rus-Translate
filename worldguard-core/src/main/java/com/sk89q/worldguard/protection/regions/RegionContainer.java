...
public abstract class RegionContainer {
    ...
    public void migrate(Migration migration) throws MigrationException {
        ...
        synchronized (lock) {
            try {
                WorldGuard.logger.info("Выгрузка и сохранение данных региона, которые загружены в данный момент...");
                unload();
                migration.migrate();
            } finally {
    ...
    protected void autoMigrate() {
        ...
        if (config.migrateRegionsToUuid) {
            ...
            try {
                ...
                WorldGuard.logger.info("Регионы сохранены после миграции UUID! Это не повторится, если только " +
                        "вы измените соответствующий параметр конфигурации в конфиге WorldGuard.");
