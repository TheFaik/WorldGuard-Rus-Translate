...
public class WorldGuardExceptionConverter extends ExceptionConverterHelper {
    ...
    public void convert(NumberFormatException e) throws CommandException {
        ...
        if (matcher.matches()) {
            throw newCommandException("Ожидалось число; строка \"" + matcher.group(1)
                    + "\" given.", e);
        } else {
            throw newCommandException("Ожидалось число; дана строка.", e);
        }
    }
    ...
    public void convert(RejectedExecutionException e) throws CommandException {
        throw newCommandException("В настоящее время в очереди слишком много задач, чтобы добавить вашу. Используйте /wg running для просмотра списка задач в очереди и в процессе выполнения.", e);
    }
