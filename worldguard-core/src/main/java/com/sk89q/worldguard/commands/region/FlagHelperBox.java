...
class FlagHelperBox extends PaginationBox {
        ...
        if (flag.usesMembershipAsDefault()) {
            builder.append(TextComponent.empty().append(TextComponent.of("*", TextColor.AQUA))
                    .hoverEvent(HoverEvent.of(HoverEvent.Action.SHOW_TEXT,
                            TextComponent.of("Это специальный флаг, который по умолчанию разрешен для участников и запрещен для тех, кто не является участником."))));
            length += monoSpace ? 1 : FlagFontInfo.getPxLength('*');
        }
        if (flag == Flags.PASSTHROUGH) {
            builder.append(TextComponent.empty().append(TextComponent.of("*", TextColor.AQUA))
                    .hoverEvent(HoverEvent.of(HoverEvent.Action.SHOW_TEXT,
                            TextComponent.of("Это специальный флаг, который переопределяет проверки строительства. (Не связано с перемещением!))"))));
            length += monoSpace ? 1 : FlagFontInfo.getPxLength('*');
        }
    ...
    private <V> void appendValueChoices(TextComponent.Builder builder, Flag<V> flag, Iterator<V> choices, @Nullable String suggestChoice) {
        ...
        while (choices.hasNext()) {
            ...
            if (maySet) {
                if (isExplicitSet) {
                    ...
                    hoverTexts.add(TextComponent.of("Установка этого флага может привести к непредвиденным последствиям.", TextColor.RED)
                            .append(TextComponent.newline())
                            .append(TextComponent.of("Пожалуйста, прочтите документацию и установите этот флаг вручную, если вы действительно намерены это сделать.")
                            .append(TextComponent.newline())
                            .append(TextComponent.of("(Подсказка: Вам не нужно устанавливать этот флаг для защиты региона!)"))));
                } else {
        ...
        if (suggestChoice != null && perms.maySetFlag(region, flag)) {
            builder.append(TextComponent.of(suggestChoice, TextColor.DARK_GRAY)
                    .hoverEvent(HoverEvent.of(HoverEvent.Action.SHOW_TEXT,
                            TextComponent.of("Нажмите, чтобы установить собственное значение", TextColor.GOLD)))
    ...
    private <V> void appendValueText(TextComponent.Builder builder, Flag<V> flag, String display, @Nullable Component hover) {
        ...
        if (maySet) {
            if (isExplicitSet) {
                ...
                hoverTexts.add(TextComponent.of("Нажмите, чтобы задать", TextColor.GOLD));
            }
        }
    ...
    private <V> Component getToolTipHint(V defVal, V currVal, boolean inherited) {
        ...
        if (inherited) {
            if (currVal == defVal) {
                valType = TextComponent.of("Наследуемые & ")
                        .append(TextComponent.of("стандарт")
                                .decoration(TextDecoration.UNDERLINED, true))
                        .append(TextComponent.of(" значение"));
            } else {
    ...
    private <V> void appendSetFlagValue(TextComponent.Builder builder, SetFlag<V> flag) {
        ...
        subName = clazz.isAssignableFrom(RegistryFlag.class)
                ...
                : subType.getClass().getSimpleName().replace("Флаг", "");
    ...
    private void appendLocationFlagValue(TextComponent.Builder builder, LocationFlag flag) {
        ...
        if (currVal == null) {
            ...
            if (defVal == null) {
                appendValueText(builder, flag, "сброшенное местоположение", null);
            } else {
    ...
    private <V extends Number> void appendNumericFlagValue(TextComponent.Builder builder, NumberFlag<V> flag) {
        ...
        appendValueChoices(builder, flag, (Iterator<V>) choices.iterator(), choices.isEmpty() ? "сброшенное число" : "[пользовательское значение]");
    }
