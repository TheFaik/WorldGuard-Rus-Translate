...
public class MemberCommands extends RegionCommandsBase {
    ...
    @Command(aliases = {"addmember", "addmember", "addmem", "am"},
            usage = "<id> <новый участник...>",
            flags = "nw:",
            desc = "Добавить нового участника в регион",
            min = 2)
    public void addMember(CommandContext args, Actor sender) throws CommandException {
        ...
        final String description = String.format("Добавление участников в регион '%s' on '%s'", region.getId(), world.getName());
        AsyncCommandBuilder.wrap(resolver, sender)
                .registerWithSupervisor(worldGuard.getSupervisor(), description)
                .onSuccess(String.format("Регион '%s' обновлен, добавлены новые участники.", region.getId()), region.getMembers()::addAll)
    ...
    @Command(aliases = {"addowner", "addowner", "ao"},
            usage = "<id> <новый владелец...>",
            flags = "nw:",
            desc = "Добавить нового владельца в регион",
            min = 2)
    public void addOwner(CommandContext args, Actor sender) throws CommandException {
        ...
        AsyncCommandBuilder.wrap(checkedAddOwners(sender, manager, region, world, resolver), sender)
                .registerWithSupervisor(worldGuard.getSupervisor(), description)
                .onSuccess(String.format("Регион '%s' обновлен новыми владельцами.", region.getId()), region.getOwners()::addAll)
            ...
            if (sender instanceof LocalPlayer) {
                ...
                if (owners.contains(player) && !sender.hasPermission("worldguard.region.unlimited")) {
                    ...
                    if (maxRegionCount >= 0 && manager.getRegionCountOfPlayer(player)
                            >= maxRegionCount) {
                        throw new CommandException("Вы уже владеете максимально допустимым количеством регионов.");
                    }
                }
            }
    ...
    public void removeMember(CommandContext args, Actor sender) throws CommandException {
        ...
        if (args.hasFlag('a')) {
            ...
            if (args.argsLength() < 2) {
                throw new CommandException("Перечислите имена для удаления или используйте -a, чтобы удалить всех.");
            }
        ...
        AsyncCommandBuilder.wrap(callable, sender)
                .registerWithSupervisor(worldGuard.getSupervisor(), description)
                .sendMessageAfterDelay("(Пожалуйста, подождите... запрос имен игроков...)")
                .onSuccess(String.format("Регион '%s' обновлен, участники удалены.", region.getId()), region.getMembers()::removeAll)
    ...
    public void removeOwner(CommandContext args, Actor sender) throws CommandException {
        ...
        AsyncCommandBuilder.wrap(callable, sender)
                .registerWithSupervisor(worldGuard.getSupervisor(), description)
                .sendMessageAfterDelay("(Пожалуйста, подождите... запрос имен игроков...)")
                .onSuccess(String.format("Пожалуйста, подождите... запрос имен игроков...", region.getId()), region.getOwners()::removeAll)
