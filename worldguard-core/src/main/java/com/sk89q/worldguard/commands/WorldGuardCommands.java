...
public class WorldGuardCommands {
    ...
    @Command(aliases = {"version"}, desc = "Узнать версию WorldGuard", max = 0)
    ...
    public void reload(CommandContext args, Actor sender) throws CommandException {
        ...
        if (!tasks.isEmpty()) {
            throw new CommandException("В данный момент есть ожидающие задачи. Используйте /wg running, чтобы сначала просмотреть эти задачи.");
        }
    ...
    @Command(aliases = {"report"}, desc = "Пишет отчет о WorldGuard", flags = "p", max = 0)
    ...
    public void report(CommandContext args, final Actor sender) throws CommandException, AuthorizationException {
        ...
        try {
            ...
            sender.print("Отчет WorldGuard записан в " + dest.getAbsolutePath());
        } catch (IOException e) {
            throw new CommandException("Не удалось записать отчет: " + e.getMessage());
        }
    ...
    @Command(aliases = {"profile"}, usage = "[-p] [-i <интервал>] [-t <фильтр потоков>] [<минут>]",
            desc = "Профилировать использование процессора сервера", min = 0, max = 1,
            flags = "t:i:p")
    ...
    public void profile(final CommandContext args, final Actor sender) throws CommandException, AuthorizationException {
        ...
        if (args.argsLength() == 0) {
            ...
            if (minutes < 1) {
                throw new CommandException("Вы должны запустить профилирование как минимум на 1 минуту.");
            } else if (minutes > 10) {
                throw new CommandException("Вы можете проводить профилирование не более 10 минут.");
            }
        }
        ...
        if (args.hasFlag('i')) {
            ...
            if (interval < 10) {
                sender.printDebug("Примечание: низкий интервал может вызвать дополнительные задержки во время профилирования.");
            }
        }
        Sampler sampler;
        ...
        synchronized (this) {
            if (activeSampler != null) {
                throw new CommandException("Профилирование уже запущено! Пожалуйста, используйте /wg stopprofile, чтобы отменить текущее профилирование.");
            }
        ...
        sender.print(TextComponent.of("Запуск профилирования CPU. Результаты будут доступны через " + minutes + " минут(-ы).", TextColor.LIGHT_PURPLE)
        ...
        worldGuard.getSupervisor().monitor(FutureForwardingTask.create(
                sampler.getFuture(), "Профилирование CPU для " + minutes + " минут(-ы)", sender));
    ...
    @Command(aliases = {"stopprofile"}, usage = "",desc = "Остановите текущее профилирование", min = 0, max = 0)
    @CommandPermissions("worldguard.profile")
    public void stopProfile(CommandContext args, final Actor sender) throws CommandException {
        synchronized (this) {
            if (activeSampler == null) {
                throw new CommandException("В настоящее время профилирование CPU не запущено.");
            }
        ...
        sender.print("Текущее профилирование CPU было отменено.");
    }
    ...
    @Command(aliases = {"running", "queue"}, desc = "Вывести список запущенных задач.", max = 0)
    ...
    public void listRunningTasks(CommandContext args, Actor sender) throws CommandException {
        ...
        if (tasks.isEmpty()) {
            ...
            MessageBox builder = new MessageBox("Запущенные задачи", new TextComponentProducer());
            builder.append(TextComponent.of("Примечание: некоторые 'запущенные' задачи могут ожидать старта.", TextColor.GRAY));
            for (Task<?> task : tasks) {
